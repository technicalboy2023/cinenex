...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    const movie = await getMovieDetail(parseInt(id, 10), region);
    return {
      title: movie.title,
...
export default async function MovieDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    const acceptLanguage = headersList.get('accept-language') || 'en-US';
    const region = acceptLanguage.split(',')[0].split('-')[1]?.toUpperCase() || 'US';
    movie = await getMovieDetail(movieId, region);
    ...
    // Instead of throwing an error (which triggers the Next.js red dev overlay), 
    // we return a standard React component acting as our error screen safely.
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-4 text-center">
        <div className="text-6xl mb-4">😢</div>
        <h1 className="text-2xl font-bold text-foreground mb-2">Failed to load data</h1>
        <p className="text-muted max-w-md mx-auto mb-6">
          The movie database is currently unreachable or the connection was unexpectedly reset.
        </p>
      </div>
    );
  }
